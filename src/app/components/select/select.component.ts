...
import { NgControl, ControlValueAccessor, UntypedFormControl, ReactiveFormsModule } from '@angular/forms';
...
import { OPTION_PARENT, OptionComponent } from '../option/option.component';
import { OptionGroupComponent } from '../option/option-group.component';
import { AbstractFormControl } from '../forms/form-control';
...
@Component({
  ...
  providers: [
    { provide: AbstractFormControl, useExisting: SelectComponent },
    { provide: OPTION_PARENT, useExisting: SelectComponent },
  ],
})
...
export class SelectComponent
  ...
  private _initKeyManager(viewOptions: any): void {
  ...
  private _handleOptionClick(option: OptionComponent): void {
    if (option.disabled) {
      return;
    }
  ...
  private _handleSingleSelection(option: OptionComponent): void {
  ...
  private _handleMultipleSelection(option: OptionComponent): void {
  ...
  private _setSelection(selectValue: any | any[]): void {
    ...
    this.stateChanges.next();
    this._cdRef.markForCheck();
    this._input.nativeElement.value = previousSelected.map(o => o.label).join(', ');
  }

  private _showFilteredOptions(): void {
    this.options.toArray().forEach((option: OptionComponent) => {
      option.hidden = false;
    });
  ...
  private _scrollToOption(option: OptionComponent): void {
