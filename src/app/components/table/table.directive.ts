...
export class TableDirective<T> implements OnInit, AfterViewInit, OnDestroy {
  @Input()
  get striped(): boolean {
  ...
  @Input() pagination: TablePaginationComponent;
  @Input() sort: TableSortDirective;
  ...
  defaultFilterFn(data: T, searchTerm: string): boolean {
    return Object.keys(data as {}).some((key: string) => {
      if (data[key as keyof T]) {
        return (data[key as keyof T] as any).toString().toLowerCase().includes(searchTerm.toLowerCase());
      }
    });
  }
  ...
  private _sortData(data: T[]): T[] {
    ...
    if (direction !== 'none') {
      return data.sort((a, b) => {
        const result = this._sortFn(a[name as keyof T] as number | string, b[name as keyof T] as number | string);
